...
public interface Apply<mu> extends Functor<mu> {

    // <*> (Control.Applicative), ap (Control.Monad)
    public <A, B> _<mu, B> ap(_<mu, F1<A, B>> fn, _<mu, A> nestedA);

    //curried version of ap
    public default <A, B> F1<_<mu, A>, _<mu, B>> ap(final _<mu, F1<A, B>> nestedFn) {
    ...
    // <* (Control.Applicative)
    public default <A, B> _<mu, A> leftSeq(_<mu, A> nestedA, _<mu, B> nestedB) {
        return lift2(F2.<A,B>constant()).$(nestedA, nestedB);
    }

    // *> (Control.Applicative)
    public default <A, B> _<mu, B> rightSeq(_<mu, A> nestedA, _<mu, B> nestedB) {
        return leftSeq(nestedB, nestedA);
    }

    //liftA2 (Control.Applicative), liftM2 (Control.Monad)
    public default <A, B, C> F2<_<mu, A>, _<mu, B>, _<mu, C>> lift2(final F1<A, F1<B, C>> fn) {
    ...
    public default <A, B, C, D> F3<_<mu, A>, _<mu, B>, _<mu, C>, _<mu, D>> lift3(final F1<A, F1<B, F1<C, D>>> fn) {
