...
public interface Foldable<µ> {
    ...
    public default <A, B> B foldMap(final Monoid<B> mb, final Function<A, B> fn, _<µ, A> nestedA) {
        return foldr((A a) -> b -> mb.dot(fn.apply(a), b), mb.identity(), nestedA);
    }

    public default <A, B> B foldr(final Function<A, Function<B, B>> fn, B b, _<µ, A> as) {
        //foldr f z t = appEndo (foldMap (Endo . f) t) z
        return foldMap(Functions.<B>endoMonoid(), fn, as).apply(b);
    }

    //This is very inefficient, please override if possible.
    public default <A, B> A foldl(final Function<A, Function<B, A>> fn, A a, _<µ, B> bs) {
    ...
    public default <A> A fold1(Semigroup<A> sa, _<µ, A> nestedA) {
        return foldMap1(sa, Functions.<A>id(), nestedA);
    }

    public default <A, B> B foldMap1(Semigroup<B> sa, Function<A, B> fn, _<µ, A> nestedA) {
    ...
    public default <A> Maybe<A> foldr1(final Function<A, Function<A, A>> fn, _<µ, A> nestedA) {
    ...
    public default <A> Maybe<A> foldl1(final Function<A, Function<A, A>> fn, _<µ, A> nestedA) {
    ...
    public default <A> List<A> toList(_<µ, A> nestedA) {
