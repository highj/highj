...
public interface Extend<W> extends Functor<W> {
    ...
    default <A> _<W, _<W, A>> duplicate(_<W, A> nestedA) {
        // duplicate = extend id
        return extend(Function.<_<W, A>>identity()).apply(nestedA);
    }

    default <A, B> Function<_<W, A>, _<W, B>> extend(final Function<_<W, A>, B> fn) {
        //extend f = fmap f . duplicate
        return x -> lift(fn).apply(duplicate(x));
    }

    //(=>=)
    default <A, B, C> Function<_<W, A>, C> cokleisli(Function<_<W, A>, B> f, Function<_<W, B>, C> g) {
        //f =>= g = g . extend f
