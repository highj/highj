...
public interface Foldable<F> {
    ...
    default <A> A fold(Monoid<A> ma, _<F, A> nestedA) {
        return foldMap(ma, Function.<A>identity(), nestedA);
    }

    default <A, B> B foldMap(final Monoid<B> mb, final Function<A, B> fn, _<F, A> nestedA) {
        return foldr((A a) -> b -> mb.apply(fn.apply(a), b), mb.identity(), nestedA);
    }

    default <A, B> B foldr(final Function<A, Function<B, B>> fn, B b, _<F, A> as) {
        //foldr f z t = appEndo (foldMap (Endo . f) t) z
        return foldMap(Functions.<B>endoMonoid(), fn, as).apply(b);
    }

    //This is very inefficient, please override if possible.
    default <A, B> A foldl(final Function<A, Function<B, A>> fn, A a, _<F, B> bs) {
    ...
    default <A> A fold1(Semigroup<A> sa, _<F, A> nestedA) {
        return foldMap1(sa, Function.<A>identity(), nestedA);
    }

    default <A, B> B foldMap1(Semigroup<B> sa, Function<A, B> fn, _<F, A> nestedA) {
    ...
    default <A> Maybe<A> foldr1(final Function<A, Function<A, A>> fn, _<F, A> nestedA) {
    ...
    default <A> Maybe<A> foldl1(final Function<A, Function<A, A>> fn, _<F, A> nestedA) {
    ...
    default <A> List<A> toList(_<F, A> nestedA) {
