 ...
 * @param <S>
 */
public interface MapApply<S> extends Apply<_<Map.µ, S>> {

    @Override
    public default <A, B> Map<S, B> map(Function<A, B> fn, _<_<Map.µ, S>, A> nestedA) {
        return Map.narrow(nestedA).map(fn);
    }

    @Override
    public default <A, B> Map<S, B> ap(_<_<Map.µ, S>, Function<A, B>> fn, _<_<Map.µ, S>, A> nestedA) {
