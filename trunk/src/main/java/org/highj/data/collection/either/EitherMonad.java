...
public class EitherMonad<S> extends EitherFunctor<S> implements Monad<__.µ<Either.µ, S>> {
    ...
    @Override
    public <A, B> Either<S, B> ap(_<__.µ<Either.µ, S>, Function<A, B>> fn, _<__.µ<Either.µ, S>, A> nestedA) {
    ...
    @Override
    public <A, B> Either<S, B> bind(_<__.µ<Either.µ, S>, A> a, Function<A, _<__.µ<Either.µ, S>, B>> fn) {
        //RightLazy m >>= k = k m
        //LeftLazy e  >>= _ = LeftLazy e
        return Either.narrow(a).<Either<S,B>>either(Either::Left, right -> Either.narrow(fn.apply(right)));
    }
}
